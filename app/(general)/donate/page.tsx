...
export const metadata: Metadata = {
  ...
  description: 'Support practical digital education and technology access in rural Nigeria.',
};
  ...
  return (
    <main className="bg-white pt-16 text-[#0d1117] transition-colors duration-300 dark:bg-[#0d1117] dark:text-white">
      <section className="relative isolate flex min-h-[476px] items-center overflow-hidden bg-[linear-gradient(157deg,#0d1117_0%,#0d2320_100%)] py-20 text-white">
        ...
        />
        <div aria-hidden="true" className="absolute inset-0 -z-10 bg-[linear-gradient(157deg,rgba(13,17,23,0.88)_0%,rgba(13,35,32,0.86)_100%)]" />
        ...
        <Reveal className="mx-auto w-full max-w-[896px] px-6 text-center">
          ...
          <p className="mx-auto mt-6 max-w-[672px] text-base leading-7 text-white/70 sm:text-xl">
            Every dollar moves us closer to a digitally literate Nigeria. See exactly what your contribution unlocks.
          </p>
        </Reveal>
      </section>
        ...
        <div className="mx-auto grid max-w-[1152px] gap-12 px-6 lg:grid-cols-[minmax(0,2fr)_333px]">
          ...
          <Reveal direction="right" delay={100} className="space-y-6">
            <aside className="rounded-2xl border border-black/[0.08] bg-[#f8fafb] p-6 transition-colors duration-300 dark:border-white/10 dark:bg-[#141d20]">
              <h2 className="font-jakarta text-lg font-extrabold leading-7">Your donation at work</h2>
              <dl className="mt-5 space-y-4">
                {donationImpact.map(([amount, impact]) => (
                  <div key={amount} className="grid grid-cols-[56px_1fr] gap-3">
                    <dt className="text-right text-sm font-bold leading-5 text-[#04af9f]">{amount}</dt>
                    <dd className="text-sm leading-[22.75px] text-[#6b7280] dark:text-white/60">{impact}</dd>
                  </div>
                ))}
              </dl>
            </aside>
            ...
            <aside className="rounded-2xl border border-black/[0.08] bg-[#f8fafb] p-6 transition-colors duration-300 dark:border-white/10 dark:bg-[#141d20]">
              <h2 className="font-jakarta text-lg font-extrabold leading-7">Other ways to give</h2>
              <ul className="mt-4">
                {otherWaysToGive.map((way, index) => (
                  <li key={way} className={index === otherWaysToGive.length - 1 ? '' : 'border-b border-black/[0.08] dark:border-white/10'}>
                    <Link
                      href="/contact"
                      className="group flex items-center gap-3 py-3 text-sm font-semibold transition-colors hover:text-[#04af9f]"
                    >
                      <span aria-hidden="true" className="size-2 rounded-full bg-[#04af9f] transition-transform duration-300 group-hover:scale-125" />
                      {way}
                    </Link>
                  </li>
                ))}
            ...
            <aside className="rounded-2xl bg-[linear-gradient(159deg,#04af9f_0%,#028f82_100%)] p-6 text-white">
              ...
              <blockquote className="mt-3 text-sm font-semibold leading-[22.75px] text-white/90">
                “The laptop from Whiz Academy changed my destiny.”
                <span className="font-black"> Emeka, 16</span>
              </blockquote>
            </aside>
          </Reveal>
        </div>
